...
#if MANIA_USE_PLUS
    if ((player->superState >= SUPERSTATE_SUPER || emeralds != 0b01111111 || player->rings < 50) && emeraldMasks != 0xFF)
        return false;
    ...
    if (Player_CheckAttacking(player, entity)) {
        ...
        if (player->characterID == ID_KNUCKLES && player->animator.animationID == ANI_GLIDE) {
            ...
            if (player->state == Player_State_AmyHeliHammer_Left || player->state == Player_State_AmyHeliHammer_Right) {
                player->state = Player_State_Air;
                RSDK.SetSpriteAnimation(player->aniFrames, 51, &player->animator, false, 0);
                if (player->velocity.y < -0x40000)
                    player->velocity.y = -0x40000;
            }

            if (player->characterID == ID_AMY && player->animator.animationID == 50) {
                player->state = Player_State_Air;
                RSDK.SetSpriteAnimation(player->aniFrames, 51, &player->animator, false, 0);
                if (player->velocity.y < -0x40000)
                    player->velocity.y = -0x40000;
...
void Player_UpdatePhysicsState(EntityPlayer *entity)
{
    ...
    switch (entity->characterID) {
        ...
        case ID_AMY: tablePtr = Player->mightyPhysicsTable; break;
#endif
    }
...
void Player_Action_TallJump(void)
{
    ...
    if (hammeranimation == 49) { // if the animation ID is 49, proceed
        if (self->isChibi ? hammerframe == 4 : hammerframe == 6) {  // if the frame ID is equal to or greater than 6 (7 w/o indexing), proceed
            self->onGround = false;
            spinning       = true;
            if (self->collisionMode == CMODE_FLOOR)
                self->position.y += self->jumpOffset;
...
void Player_FinishedReturnToPlayer(EntityPlayer *player, EntityPlayer *leader)
{
    ...
    if (!SizeLaser || player->state == Player_State_FlyToPlayer) {
        ...
        switch (player->characterID) {
            ...
            case ID_AMY: player->aniFrames = Player->amyFrames; break;
#endif
        }
    ...
    if (!self->onGround) {
        ...
        switch (self->characterID) {
            ...
            case ID_KNUCKLES:
                ...
                break;
            case ID_AMY: self->groundVel = 0; break;
...
void Player_Input_P2_AI(void)
{
    ...
    if (self->state == Player_State_TailsFlight && leader->state == Player_State_FlyCarried) {
        ...
        controller->keyA.press = leader->jumpPress;
        controller->keyB.press = leader->jumpPress;
    }
